...
<Tabs items={["Claude Code", "Cursor", "Codex", "Any MCP client"]}>
  ...
  <Tab value="Codex">
    ...
    Or the remote MCP server with an API key (Codex only reads the
    nested `http_headers` table — not `headers`):
    ...
    For OAuth instead of an API key, omit `http_headers` and run
    `codex mcp login memax` once.

...
func main() {
	...
	enableDistillation := flag.Bool("distill", true, "Enable query distillation when the LLM API key is configured")
	enableQA := flag.Bool("qa", false, "Enable QA hypothesis generation (requires an LLM API key)")
	qaCutoffs := flag.String("qa-cutoffs", "10", "Comma-separated retrieval cutoffs used for QA generation when -qa is enabled")
	qaModel := flag.String("qa-model", anthropic.StrongModel, "Model for QA generation")
	qaMaxTokens := flag.Int("qa-max-tokens", 64, "Max output tokens for QA generation")
	enableJudge := flag.Bool("judge", false, "Enable LLM-as-judge scoring for generated answers (requires -qa and an LLM API key)")
	judgeModel := flag.String("judge-model", anthropic.StrongModel, "Model for LLM-as-judge scoring")
	...
	if *dataset == "" {
		...
		fmt.Fprintln(os.Stderr, "Optional: ANTHROPIC_API_KEY + ANTHROPIC_BASE_URL (LLM gateway, e.g. OpenRouter, for distillation and --qa)")
		flag.PrintDefaults()
		os.Exit(1)
	}
